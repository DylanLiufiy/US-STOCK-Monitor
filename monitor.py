...
# 2. 🟢 极致精简监控池：只留下 2 大大盘基金 + 4 只最硬核的王牌个股
MONITOR_POOL = {
    "VOO":  {"name": "标普500-ETF", "min": 5.0, "max": 10.0, "risk_pct": 25.0}, # 提高权重
    "QQQ":  {"name": "纳指100-ETF", "min": 5.0, "max": 10.0, "risk_pct": 25.0}, # 提高权重
    "NVDA": {"name": "英伟达",     "min": 5.0, "max": 10.0, "risk_pct": 12.0}, # 聚焦重仓
    "MSFT": {"name": "微软",       "min": 5.0, "max": 10.0, "risk_pct": 10.0}, # 聚焦重仓
    "GOOG": {"name": "谷歌-C",     "min": 5.0, "max": 10.0, "risk_pct": 10.0}, # 聚焦重仓
    "PLTR": {"name": "Palantir",   "min": 6.0, "max": 12.0, "risk_pct": 8.0}   # 聚焦重仓
}
...
def check_drawdown():
        ...
        try:
            ...
            if min_drop <= drawdown <= max_drop:
                ...
                body = (
                    ...
                    f"最新价: ${current_price:.2f} (已回撤 -{drawdown:.2f}%)\n"
                    f"1. 动作: 买入 / BUY 🟢\n"
                    f"2. 代码: {ticker}\n"
    ...
    if not alert_triggered:
        print("目前盘中没有任何核心资产触及加仓线，系统保持静默。")
