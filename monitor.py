import os
import sys
import time
import urllib.parse
from datetime import datetime, timezone, timedelta
from concurrent.futures import ThreadPoolExecutor, as_completed
import requests
import pandas as pd
import yfinance as yf
from finvizfinance.screener.overview import Overview

# ==========================================
# 1. 核心配置与环境变量优化
# ==========================================
BARK_KEY = os.environ.get("BARK_KEY")
DEEPSEEK_API_KEY = os.environ.get("DEEPSEEK_API_KEY")

# 策略控制常量
VOLUME_MULTIPLIER = 3.0
MIN_STOCK_PRICE = 2.0
MIN_3DAY_AVG_TURNOVER = 5_000_000
SINGLE_SNIPER_BUDGET_USD = 800
MAX_SCAN_COUNT = 45
MAX_WORKERS = 8  # 保持 8 个并发，既能提速 5-8 倍，又不易触发布局被雅虎封锁

# ==========================================
# 2. 智能化 AI 分析引擎
# ==========================================
def ask_ai_investigation(ticker_symbol: str, price: float, mult: float) -> str:
    """
    🧠 AI 事件驱动引擎：修复了原始代码中错误的 API Endpoints (https://deepseek.com)
    """
    if not DEEPSEEK_API_KEY:
        return "ℹ 未配置 AI 情绪钥匙，系统启动纯技术面拦截通告。"
        
    try:
        t = yf.Ticker(ticker_symbol)
        news_list = t.news[:3] if t.news else []
        news_titles = [n.get('title', '') for n in news_list if n.get('title')]
        news_context = " | ". join(news_titles) if news_titles else "暂无近两日公开突发大事件"
        
        prompt = (
            f"作为美股顶级游资席位量化主管，请用一句话（100字内，干货大白话）刺破美股小盘股【{ticker_symbol}】"
            f"突然触发机构暴力抢筹（股价${price:.2f}，成交量狂飙{mult:.1f} 倍）的底层导火索。"
            f"结合最新的新闻流：{news_context}。告诉老手这是主力拉高出货、还是有重大基本面拐点（如FDA、并购、财报超预期），"
            f"并给出一句核心行动策略。"
        )
        
        headers = {
            "Authorization": f"Bearer {DEEPSEEK_API_KEY}",
            "Content-Type": "application/json"
        }
        data = {
            "model": "deepseek-chat",
            "messages": [{"role": "user", "content": prompt}],
            "temperature": 0.3
        }
        
        # 修正：DeepSeek 官方 API 请求地址为 ://deepseek.com
        response = requests.post(
            "https://://deepseek.com/chat/completions", 
            json=data, 
            headers=headers, 
            timeout=12
        )
        if response.status_code == 200:
            return response.json()['choices'][0]['message']['content'].strip()
        else:
            return f"💡 [系统提示] AI 服务状态码异常({response.status_code})，请跟随技术趋势，定死止损。"
    except Exception as e:
        print(f"⚠ AI 短线爆破情绪分析异常: {e}")
        return "💡 [系统提示] 游资席位异动剧烈，请严格跟随一分钟K线趋势，止损定死。"

# ==========================================
# 3. 自动化全美小盘股池获取
# ==========================================
def fetch_us_stock_universe():
    """
    🛰 自动穿透全美股小盘成分股，加入多字段备份策略
    """
    print("🛰 正在全量扫描全美股数千家挂牌公司...")
    backup_list = ["AXTI", "WOLF", "XFABF", "VICR", "AAOI", "RMBS", "ALGM", "LSCC", "CEVA", "ACMR", "PXLW", "INDI", "RAMP", "HIMX"]
    try:
        fscreen = Overview()
        fscreen.set_filter(filters_dict={'Market Cap.': 'Small ($300mln to $2bln)', 'Country': 'USA'})
        try:
            df = fscreen.get_screener_platform(layout='Overview')
        except AttributeError:
            df = fscreen.screener_view()
            
        if df is not None and not df.empty:
            df.columns = df.columns.str.strip()
            for col in ['Ticker', 'ticker', 'TICKER']:
                if col in df.columns:
                    return df[col].astype(str).str.strip().tolist()
        return backup_list
    except Exception as e:
        print(f"⚠ 获取 Finviz 股票池异常: {e}，将启用兜底池。")
        return backup_list

# ==========================================
# 4. 单股核心量化透视算力（核心重构）
# ==========================================
def process_single_stock(ticker_symbol: str):
    """
    针对单只股票的解耦核心计算，支持多线程高并发调度。
    """
    try:
        ticker = yf.Ticker(ticker_symbol)
        # prepost=True 拦截盘前对敲与暗池拉升
        hist = ticker.history(period="40d", prepost=True)
        if hist.empty:
            return None
            
        hist = hist.dropna(subset=['Close', 'Volume'])
        if len(hist) < 32: 
            return None
            
        # 严格的时间序列穿透：倒数第1天到第3天
        past_30_days_volume = hist['Volume'].iloc[-32:-2]
        avg_volume_30d = past_30_days_volume.mean()
        if avg_volume_30d <= 0: 
            return None
            
        price_t0, vol_t0 = float(hist['Close'].iloc[-1]), float(hist['Volume'].iloc[-1])
        price_t1, vol_t1 = float(hist['Close'].iloc[-2]), float(hist['Volume'].iloc[-2])
        price_t2, vol_t2 = float(hist['Close'].iloc[-3]), float(hist['Volume'].iloc[-3])
        
        turnover_t0 = price_t0 * vol_t0
        turnover_t1 = price_t1 * vol_t1
        turnover_t2 = price_t2 * vol_t2
        avg_3day_turnover = (turnover_t0 + turnover_t1 + turnover_t2) / 3
        
        # 指标校准
        mult_t1 = vol_t1 / avg_volume_30d
        mult_t2 = vol_t2 / avg_volume_30d
        current_multiplier = vol_t0 / avg_volume_30d
        
        # 触发门槛过滤
        if price_t0 < MIN_STOCK_PRICE or avg_3day_turnover < MIN_3DAY_AVG_TURNOVER: 
            return None
            
        if mult_t1 >= VOLUME_MULTIPLIER or current_multiplier >= VOLUME_MULTIPLIER:
            # 经典真实波幅 ATR 计算
            high_low = hist['High'] - hist['Low']
            high_close = (hist['High'] - hist['Close'].shift()).abs()
            low_close = (hist['Low'] - hist['Close'].shift()).abs()
            tr = high_low.combine(high_close, max).combine(low_close, max)
            atr_val = tr.rolling(window=14).mean().iloc[-1]
            
            suggested_shares = SINGLE_SNIPER_BUDGET_USD / price_t0 if price_t0 > 0 else 0
            stop_loss_price = price_t0 - (2.5 * atr_val) if atr_val > 0 else price_t0 * 0.91
            trigger_mult = max(mult_t1, current_multiplier)
            
            # 引入 AI 进行深度异动原因剖析
            ai_intelligence = ask_ai_investigation(ticker_symbol, price_t0, trigger_mult)
            
            # 返回需要推送的结构化数据
            return {
                "ticker": ticker_symbol,
                "price_t0": price_t0,
                "mult_t1": mult_t1,
                "mult_t2": mult_t2,
                "current_multiplier": current_multiplier,
                "avg_3day_turnover": avg_3day_turnover,
                "suggested_shares": suggested_shares,
                "stop_loss_price": stop_loss_price,
                "ai_intelligence": ai_intelligence
            }
    except Exception as e:
        # 单股异常静默，确保总队列流畅进行
        pass
    return None

# ==========================================
# 5. 消息管道分发层
# ==========================================
def push_bark_notification(title: str, content: str, target_url: str = None, group: str = "系统状态", sound: str = "none"):
    """
    Bark 消息一站式标准分发
    """
    if not BARK_KEY:
        return
    try:
        encoded_title = urllib.parse.quote_plus(title)
        encoded_content = urllib.parse.quote_plus(content)
        url = f"https://api.day.app/{BARK_KEY}/{encoded_title}/{encoded_content}?group={group}&sound={sound}&isArchive=1"
        if target_url:
            url += f"&url={urllib.parse.quote_plus(target_url)}"
            
        requests.get(url, timeout=10)
    except Exception as e:
        print(f"⚠ Bark 消息推送失败: {e}")

# ==========================================
# 6. 核心并发调度主策略
# ==========================================
def execute_all_us_strategy(dynamic_stocks):
    # 修正：采用无时区警告的优雅 UTC+8 时间计算
    bj_time = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=8)))
    bj_hour = bj_time.hour
    
    # 限制仅在复盘时段执行（北京时间 08:00 - 22:00）
    if not (8 <= bj_hour < 22): 
        print("⏰ 当前非指定盘后复盘时间段，脚本安全跳过。")
        return
        
    if not dynamic_stocks: 
        return
        
    target_stocks = dynamic_stocks[:MAX_SCAN_COUNT]
    print(f"🚀 [多线程增效版] 启动全美股前 {len(target_stocks)} 只小盘股量价穿透过滤...")
    
    triggered_count = 0
    
    # 利用线程池大幅降低 I/O 等待，全速前进
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_ticker = {executor.submit(process_single_stock, ticker): ticker for ticker in target_stocks}
        
        for future in as_completed(future_to_ticker):
            res = future.result()
            if res is None:
                continue
                
            triggered_count += 1
            # 修正：原代码中拼错了雅虎财经的个股详情地址（https://yahoo.comAAPL -> ://yahoo.com）
            yahoo_finance_url = f"https://yahoo.com{res['ticker']}"
            
            push_title = f"🚨 商业席位爆破：【{res['ticker']}】惊现游资机构超级扫货流！"
            push_content = (
                f"🏷 【当前系统阶段】: 🧪 2026实战模拟推演期\n"
                f"------------------------\n"
                f"🧠 【AI 事件驱动· 核心因果深度剖析】:\n"
                f"🗣 {res['ai_intelligence']}\n"
                f"------------------------\n"
                f"💰 实时价: ${res['price_t0']:.2f} (包含盘前盘后深度追踪) | 昨日放量 {res['mult_t1']:.1f} 倍 / 今日实时 {res['current_multiplier']:.1f} 倍\n"
                f"💎 资金深度检查: 近3日游资滚存平均日成交额达到 【 ${res['avg_3day_turnover']:,.0f} 美元 】\n"
                f"📈 动能连贯性: 前日量 {res['mult_t2']:.1f} x ➔ 昨日爆破 {res['mult_t1']:.1f} x ➔ 今日承接 {res['current_multiplier']:.1f} x\n"
                f"🎯 【全美高流动性量化突击单】:\n"
                f"💵 拨发轻仓突击子弹: 【 $800 美元 】 ➔ 建议执行配额: 【 {res['suggested_shares']:.0f} 股 】\n"
                f"🛑 【ATR 动态游资防御线】: 止损价精准锁死在 【 ${res['stop_loss_price']:.2f} 】(包含盘前最高最低价测算)\n"
                f"------------------------\n"
                f"📝 提示: 手机端点击此卡片可直接一步唤醒雅虎财经看盘。"
            )
            
            print(f"🎯 发现游资异动股: {res['ticker']} | 触发推送...")
            push_bark_notification(
                title=push_title, 
                content=push_content, 
                target_url=yahoo_finance_url, 
                group="全美股主力爆破", 
                sound="electronic"
            )
            time.sleep(1.0)  # 防止发送请求过于频密
            
    if triggered_count == 0:
        push_bark_notification(
            title="🟢 短线爆破雷达：安全站岗中",
            content="全美股大数据穿透扫描结束（包含盘前全量穿透）。今日高流动性标的未发生机构暗池突发爆量。游资线无风险。",
            group="系统状态"
        )
